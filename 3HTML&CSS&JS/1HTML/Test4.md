- ## 表单
  ...
  - #### `<input>`
    ...
    - ##### 类型：
      ...
      **（14）range**
      ...
      上面代码会在0～100之间产生11个刻度。其中，`0%`、`50%`和`100%`三个位置会有文字提示，不过**浏览器很可能不支持**。
  ...
  - #### `<output>`
    ...
    `<output>`是 HTML5 引入的一个语义化标签，**用于表示计算或用户操作的结果**。它通常与表单元素（如 `<input>`、`<select>`）结合使用，通过 JavaScript 动态显示计算结果或操作反馈。
  ...
  - #### `<progress>`
    ...
    ```html
    <progress id="file" max="100" value="70">当前进度70%（在不支持progress的浏览器中显示）</progress>
    ```
  
    注意，`<progress>`元素的子元素，正常情况下不会显示。只有在浏览器不支持`<progress>`时才会显示。
