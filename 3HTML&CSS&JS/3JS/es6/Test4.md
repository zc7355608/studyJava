- ## 对象的扩展
  ...
  5. #### 对象的扩展运算符
     ...
     1. ##### 扩展运算符
        ...
        对象的扩展运算符（`...`）用于取出参数对象的所有可遍历属性，拷贝到当前对象之中。不同于数组等可迭代的结构，扩展运算符用于对象的话，底层是基于`Object.assign()`的。

        > 在 ES2018 之前，对一个没有实现迭代器接口的数据结构使用扩展运算符就会报错（比如普通对象`{}`）。
...
- ## Object 新增的静态方法
  ...
  - `Object.is()`：ES5 比较两个值是否相等，只有两个运算符：相等运算符（`==`）和严格相等运算符（`===`）。它们都有缺点，前者会自动转换数据类型，后者的`NaN`不等于自身，以及`+0`等于`-0`。JS 缺乏一种运算，在所有环境中，只要两个值是一样的，它们就应该相等。
