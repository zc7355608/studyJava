...
- ## JS语言的历史
  ...
  - ### 诞生
    ...
    > JS 因为互联网而生，紧跟着浏览器的出现而问世。回顾它的历史，就要从浏览器的历史讲起。
    >
    ...
    >
    > Netscape 公司的这种浏览器脚本语言，最初名字叫做 Mocha，1995年9月改为 LiveScript。12月，Netscape 公司与 Sun 公司（Java 语言的发明者和所有者）达成协议，后者允许将这种语言叫做 JS。这样一来，Netscape 公司可以借助 Java 语言的声势，而 Sun 公司则将自己的影响力扩展到了浏览器。
    >
    > 之所以起这个名字，并不是因为 JS 本身与 Java 语言有多么深的关系（事实上，两者关系并不深，详见下节），而是因为 Netscape 公司已经决定，使用 Java 语言开发网络应用程序，JS 可以像胶水一样，将各个部分连接起来。当然，后来的历史是 Java 语言的浏览器插件失败了，JS 反而发扬光大。
    >
    > 1995年12月4日，Netscape 公司与 Sun 公司联合发布了 JS 语言，对外宣传 JS 是 Java 的补充，属于轻量级的 Java，专门用来操作网页。
    >
    > 1996年3月，Navigator 2.0 浏览器正式内置了 JS 脚本语言。

  - ### JavaScript 与 Java 的关系

    > 这里专门说一下 JS 和 Java 的关系。它们是两种不一样的语言，但是彼此存在联系。
    >
    > JS 的基本语法和对象体系，是模仿 Java 而设计的。但是，JS 没有采用 Java 的静态类型。正是因为 JS 与 Java 有很大的相似性，所以这门语言才从一开始的 LiveScript 改名为 JS。基本上，JS 这个名字的原意是“很像Java的脚本语言”。
    >
    > JS 语言的函数是一种独立的数据类型，以及采用基于原型对象（prototype）的继承链。这是它与 Java 语法最大的两点区别。JS 语法要比 Java 自由得多。
    >
    > 另外，Java 语言需要编译，而 JS 语言则是运行时由解释器直接执行。
    >
    > 总之，JS 的原始设计目标是一种小型的、简单的动态语言，与 Java 有足够的相似性，使得使用者（尤其是 Java 程序员）可以快速上手。
