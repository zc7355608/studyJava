  ...
  - ##### 变量：
    ...
    **注意：JS 区分大小写。**`A`和`a`是两个不同的变量。
    ...
    2. 全局变量和局部变量：
       ...
       - 对于顶层函数来说，函数外部声明的变量就是**全局变量（global variable）**。全局变量可以在任何地方使用，作用域是全局。浏览器打开时空间就开辟，直到浏览器关掉该页面才销毁，生命周期较长耗费内存，尽量少用。（**`var`声明的全局变量相当于直接给全局对象`window/global`上添加属性**）
       - 在函数体内部声明的变量是**局部变量**，作用域是当前函数中。局部变量在函数被调用时内存才开辟，函数执行结束一般空间就释放了（不一定）。
    ...
    6. 使用`var`重新声明一个已经存在的变量，是无效的。如果第二次声明的时还进行了赋值，则会覆盖掉前面的值。
  ###### var的变量提升：
...
- ##### 标识符：
  ...
  标识符（identifier）指的是用来识别各种值的合法名称。最常见的标识符就是变量名，以及后面要提到的函数名。JS 语言的标识符对大小写敏感，所以`a`和`A`是两个不同的标识符。
    ...
    标识符的命名规则：由字母（Unicode 字母）、数字、下划线（`_`）、美元符号（`$`）组成，且不能以数字开头。
...
- ## 数据类型
  ...
  除了`object`对象类型，其他的统一被称为原始类型（primitive type）的值，即它们是最基本的数据类型，不能再细分了。`object`对象类型称为合成类型（complex type）的值，因为一个对象往往是多个原始类型的值的合成，可以看作是一个存放各种值的容器，其实保存的是该对象的内存地址。对象是最复杂的数据类型，又可以分成三个子类型：
  ...
  - #### `typeof`运算符
    ...
    **注意：**
    ...
    - 如果一个变量没有用`var`命令声明，直接使用就会报错。但是，放在`typeof`后面，就不报错了，而是返回`undefined`。（**ES6之前`typeof`语句永远不会发生错误**）
  ...
  - #### `Number` 类型
    ...
    这就是说，JS 语言的底层根本没有整数，所有数字都是小数（64位双精度浮点数）。容易造成混淆的是，**某些运算只有整数才能完成，此时 JS 会自动把64位浮点数，转成32位整数，然后再进行运算**。由于浮点数不是精确的值，所以**涉及小数的比较和运算要特别小心**。
    ...
    - **数值精度：**由于浮点数采用国际标准 IEEE754（64位双精度），因此精度最多只能到53个二进制位，这意味着，绝对值小于2的53次方的整数，即±2^53之内都可以精确表示。
      ...
      > 由于2的53次方是一个16位的十进制数值，所以简单的法则就是：**JS 对不超过15位有效数字的十进制整数都可以精确处理**。
    ...
    - **数值的表示法：**
      ...
      - JS 的数值字面量可以用二进制（`0b`或`0B`）、十进制、八进制（以`0`开头表示，已弃用）、十六进制（`0X`或`0x`）表示。默认情况下，JS 内部会自动将八进制、十六进制、二进制转为十进制。（ES5的严格模式以及ES6的新语法中，八进制数字必须以`0o`或`00`开头）
    ...
    - **特殊数值：**JS 提供了几个特殊的数值。
      ...
      - 正负无穷（Infinity）：`Infinity`表示“无穷”，用来表示两种场景。一种是一个正的数值太大，或一个负的数值太小，无法表示；另一种是非0数值除以0，得到`Infinity`。
        ...
        > - 由于数值正向溢出（overflow）、负向溢出（underflow）和被`0`除，JS 都不报错，所以**单纯的数学运算几乎没有可能抛出错误。**
  ...
  - #### `String` 类型
    ...
    - 反斜杠（`\`）在字符串内有特殊含义，用来表示一些特殊字符，所以又称为转义符。需要用反斜杠转义的特殊字符，主要有下面这些不可打印字符：（如果在非特殊字符前面使用反斜杠转义，则该转义字符`\`会被忽略）
      ...
      >
      > 注意：使用十进制 Unicode 码点来表示字符，只能通过ES6提供的 `String.fromCodePoint()` 或 `String.fromCharCode()` 函数来写。
    ...
    - （ES6）JS 中字符串的连接，除了用+/+=号，还有一种**模板字符串**的方式。语法：外面用\`\`号，里面`${变量/表达式}`来完成字符串的拼接。（并且这种方式支持多行字符串）
  
    - JS 用 Unicode 字符集。不仅以 Unicode 储存字符，还允许直接在程序中用 Unicode 码点表示字符，即将字符写成`\uxxxx`的形式，其中`xxxx`代表该字符的 Unicode 码点。比如，`\u00A9`代表版权符号。输出时所有字符都会转成字面形式。
  
    - 我们还需要知道，每个字符在 JS 内部都是以16位（即2个字节）的 UTF-16 格式储存。也就是说，JS 的每个字符长度固定为16位，即2个字节。
  
      > 但是，UTF-16 有两种长度：对于码点在`U+0000`到`U+FFFF`之间的字符，长度为16位（即2个字节）；对于码点在`U+10000`到`U+10FFFF`之间的字符，长度为32位（即4个字节）。而且前两个字节在`0xD800`到`0xDBFF`之间，后两个字节在`0xDC00`到`0xDFFF`之间（代理对）。举例来说，码点`U+1D306`对应的字符为`𝌆`，它写成 UTF-16 就是`0xD834 0xDF06`。
      >
      ...
      > UTF-16 使用**代理对**（Surrogate Pair）机制来表示码点大于 `U+FFFF` 的字符（即补充平面字符），而将 `0xD800` 到 `0xDFFF` 之间的范围专门保留用于这一目的。具体来说：（代理(Surrogate)是专属于UTF-16编码方式的一种机制，UTF-8和UTF-32是不用代理的）
      >
      > 1. UTF-16 是一种变长编码：
      >
      >    - **基本多文种平面（BMP，`U+0000`~`U+FFFF`）**：直接用 16 位表示。
      >    - **补充平面（`U+10000`~`U+10FFFF`）**：用两个 16 位单元（共 32 位）表示，称为**代理对**。
      >
      > 2. 为什么需要代理对？
      >
      >    - 为了让UTF-16能继续编码基本平面BMP后面的增补平面SP中的码点值，于是扩展了UTF-16编码方式。
      >    - Unicode 的码点范围是 `U+0000`~`U+10FFFF`，但单个 16 位单元只能表示 `0x0000`~`0xFFFF`（即 BMP）。
      >
      >    - **问题**：如何用两个 16 位单元表示 `U+10000`~`U+10FFFF`，同时避免与 BMP 的编码冲突？
      >
      > 3. **解决方案**：划出两个特殊范围作为代理区
      >
      >    - **高代理区（High Surrogate）**：`0xD800`~`0xDBFF`
      >    - **低代理区（Low Surrogate）**： `0xDC00`~`0xDFFF`
    ...
    - Base64 转码：
      ...
      所谓 Base64 就是一种编码方法，可以将二进制数据转成 64 个可打印 ASCII 字符的形式。使用它的主要目的，不是为了加密，而是为了不出现特殊字符，简化程序的处理。
  
      > JS 原生提供两个和 Base64 相关的方法：
      >
      > - `btoa()`：任意值转为 Base64 编码（Byte To Ascii）
      > - `atob()`：Base64 编码转为原来的值（Ascii To Byte）
      >
      > ```js
  ...
  - #### `object` 对象
    ...
    **语法：**
    ...
    - ##### `for...in` 遍历对象：（无序的遍历）
      ...
      `for...in`循环用来遍历一个对象的全部属性。由于对象没有像数组那样的`length`属性，所以普通for无法遍历对象，而且对象中属性是**无序的**，所以目前我们遍历对象**只能用`for in`遍历对象**。用法：
      ...
      >
      > 还有，数组也是对象，因此`for..in`也可以遍历数组，但是不推荐。
    
    - ##### 对象拷贝：
    
      - 浅拷贝：可以用ES6的`Object.assign(o, obj)`或`const o = { ...obj }`展开运算符，都可以实现对象浅拷贝。
      ...
      - 深拷贝：
        ...
        - 使用第三方库提供的工具函数，比如`lodash`提供的`_.cloneDeep(obj)`函数：（lodash是一个一致性、模块化、高性能的JS实用工具库，我们直接引入它的JS文件，内部函数都以`_.`开头）
        ...
        - （常用）一个更聪明的方法，通过`JSON.stringify(obj)`：先将对象转成JSON串，然后用`JSON.parse(jsonStr)`将JSON串再解析成JS对象。
    
          > 但这种方式有局限性：
          >
          > - 无法拷贝 `undefined`、`Function`、`Symbol`
          > - 无法拷贝循环引用
          > - 无法拷贝 `Date`（会变成字符串）、`RegExp`（变成空对象）、`Map`、`Set` 等特殊对象
      
        - （ES6）浏览器环境下，有一个全局函数`structuredClone`，它使用[结构化克隆算法](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Workers_API/Structured_clone_algorithm)将给定的值进行深拷贝。该方法还支持把原值中的[可转移对象](https://developer.mozilla.org/zh-CN/docs/Web/API/Web_Workers_API/Transferable_objects)*转移*（而不是拷贝）到新对象上。可转移对象与原始对象分离并附加到新对象；它们将无法在原始对象中被访问。
      ...
      - 对象拷贝如果要涉及原型链，也就是要满足以下两点：（这里不区分深浅拷贝）
        ...
        > 注意，**拷贝对象如果要保留原型链，可能带来以下问题**：（无论是深还是浅）
        >
        > 1. **构造函数副作用**：某些构造函数可能有复杂逻辑
        > 2. **性能开销**：需要维护原型关系
        > 3. **内存泄漏**：原型链可能引用大量方法
  ...
  - #### `Function` 函数
    ...
    3. ##### `Function` 构造函数（很少用）
       ...
       >
       > 这种方式创建的函数，它的词法作用域固定为全局环境（准确地说是当前Realm的全局环境），不会捕获你当前代码位置的局部变量，所以它拿不到闭包里的局部。
       >
       > 并且这种方式的性能和安全性较差，本质上类似动态执行字符串的eval函数，一般不推荐使用。
       
    
    注意：在 JS 中，**所有函数都是 `Function` 构造函数的实例**，包括您通过字面量定义的 `function a() {}`和`() => {}`。并且**Function 是 Object 的子类型**，因此**函数其实也是对象**。
    ...
    - ##### 函数内部的 `arguments` 对象
      ...
      - JS 中，所有函数/方法中都有一个变量`arguments`（箭头函数中没有`arguments`）。该对象中包含了函数运行时的所有参数，它是一个**伪数组**，`arguments[0]`就是第一个参数，`arguments[1]`就是第二个参数，以此类推。
        ...
        > **伪数组（对象）**：
        >
        > - 如果一个对象的所有键名都是正整数或零，并且有`length`属性，那么这个对象就很像数组，语法上称为“类似数组的对象”（array-like object），也就是**伪数组**。
        >
        > - 伪数组没有数组对象的`push/pop/map`等方法，要遍历只能用`for`循环。并且伪数组的`length`属性需要手动维护，和数字键名不会关联。
        >
        ...
        >   ```
        >   
        >   > 为什么`slice`方法能把伪数组转换成真数组？因为`slice`并不依赖数组的内部结构，它只依赖2点：
        >   >
        >   > - 对象上有`length`属性
        >   > - 能通过数字键访问元素（如：`obj[0], obj[2], ...`）
        >   >
        >   > （ES6）不过现在更常用的方式是：
        >   >
        >   > - `Array.form(arrayLike)`
        >   > - `const arr = [...arrayLike]`（需要可迭代对象）
        >
    ...
    - ##### 闭包
      ...
      ###### 闭包：闭包（closure）是 JS 语言的一个难点及特色，很多高级应用都要依靠闭包实现。
  ...
  - #### `Array` 数组
    ...
    - 并且由于 `Array` 是`Object` 的子类型。因此**数组本质上也是一种特殊的对象**，`typeof`运算符会返回数组的类型是`object`。
    ...
    - 可以通过数组的`length`属性，获取数组的成员数量。该属性是一个动态的值，等于键名中的最大整数加上`1`。
      ...
      > JS 使用一个32位整数，保存数组的元素个数。这意味着，数组成员最多只有 4294967295 个（2³² - 1）个，也就是说`length`属性的最大值就是 4294967295。
      >
      > 如果你的键不在这个范围内（或者不是整数形式），就不会当作数组的元素，而是（数组）对象上的普通属性。
    
    - 数组的数字键名不需要连续，`length`属性的值总是比最大的那个整数键大`1`。这种数组被称为**稀疏数组**，不连续的位置叫做数组的**空位（hole）**。
    ...
    **判断数组某个位置是否是空位**：`in`运算符或ES6的`arr.hasOwnProperty(i)`，返回`false`说明是空位。
    
    **注意：**
    
    数组的某个位置是空位，与某个位置是`undefined`，是不一样的。如果是空位，使用数组的`forEach/map/filter`方法、`for...in`结构、以及`Object.keys`方法进行遍历，空位都会被跳过；如果某个位置是`undefined`，遍历的时候就不会被跳过。
    
    这就是说，空位就是数组没有这个元素，所以不会被遍历到，而`undefined`则表示数组有这个元素，值是`undefined`，所以遍历不会跳过。
    
    虽然大部分数组方法都会跳过空位不进行遍历，比如`forEach/map/filter`，但是`for..of`、使用`length`进行`for`循环都会遍历到数组的空位。
