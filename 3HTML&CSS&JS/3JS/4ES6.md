...
- ### Rest参数
  ...
  > ES6推荐使用`rest`参数来代替`arguments`动态参数，以后参数不确定的时候就用`rest`参数，只能出现在参数表最后。语法：
...
- ### 类class
  ...
  class Woman extends Person {
      ...
      say(){//将父类的同名方法进行覆盖
          //不能调用父类的同名方法的super.say()
          console.log('我是子类的hello')
      }
  }
  ```
  
  > 对象中的get和set方法：读或改对象的某个同名属性时（可以没有该属性），会调用类中的同名get方法和set方法
...
- ### ES6模块化：
  ...
  > 模块化是指，将一个大的程序文件，拆分成多个小文件，然后将这些文件导入到某个文件中进行使用。类似Java的导包
  >
  > 使用模块化有很多好处：
  >
  > 1. 防止命名冲突
  ...
  ```javascript
  //将该文件中的数据进行逐一暴露。暴漏的是一个Module对象，里面有school和find属性
  ...
  //默认暴露的方式：这种方式是将后面的数据，暴露给Module对象中的default属性上了，此时default对象中有school和find属性
  //一个js文件只能使用1次默认暴露
  export default {
  	let school = '蓝翔',
      function find(){
  ...
  > 默认导入和按需导入我们怎么选择呢？由于默认导入会将暴露的所有数据都加载，而按需导入只导入部分数据，所以如果我们需要模块中所有数据，就默认导入，否则就按需导入。
  >
  > （如果某些浏览器不支持ES6的特性，那么需要通过Babel工具将ES6的语法编译转换成ES5的语法）
