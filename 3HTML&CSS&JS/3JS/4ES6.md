...
- ### Promise
  ...
  - #### Promise的基本使用：
    ...
    1. 使用Promise构造函数来实例化一个promise对象，用于封装异步操作：
       ...
       > - `resolve(v)`和`reject(v)`执行后会将当前promise对象的状态分别设置为成功和失败，且promise对象中会保存成功或失败的结果值。
  ...
  - #### Promise对象的状态和结果值：
    ...
    >
    > - 无论状态变为成功还是失败，promise对象中都会保存一个结果值（一般成功结果值叫value、失败结果值叫reason）。它是promise对象中的一个私有属性`[[PromiseResult]]`。只有执行了resolve()和reject()函数才可以修改该结果值。
    >
  ...
  - #### Promise的关键问题：
    ...
    - **Promise的异常穿透**
      ...
      > 当使用then()进行多次链式调用时，可以在最后去指定一个失败的回调即可，中间的then()不需要指定失败的回调。这样其中任何一层的异步任务失败了，都会返回一个失败的promise对象，没人处理后层层传递，直到传递到最后的catch()方法中处理失败的promise。
