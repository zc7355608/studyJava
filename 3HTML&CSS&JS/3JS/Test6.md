- ## 事件(to learn)
  ...
  - #### 鼠标事件
    ...
    - ##### `MouseEvent` 接口的实例属性：
      ...
      - `MouseEvent.altKey`，`MouseEvent.ctrlKey`，`MouseEvent.metaKey`，`MouseEvent.shiftKey`：
        ...
        - `altKey`属性：Alt 键
        - `ctrlKey`属性：Ctrl 键
        - `metaKey`属性：Meta 键（Mac 键盘是 `Command` 键，Windows 键盘是 `Win` 键）
        - `shiftKey`属性：Shift 键
  ...
  - #### 其他常见事件
    ...
    - ##### `session` 历史事件：
      ...
      - `pageshow` 事件，`pagehide` 事件：
        ...
        第一次加载时，它的触发顺序排在`load`事件后面。从缓存加载时，`load`事件不会触发，因为网页在缓存中的样子通常是`load`事件的监听函数运行后的样子，所以不必重复执行。同理，如果是从缓存中加载页面，网页内初始化的 JS 脚本（比如 `DOMContentLoaded` 事件的监听函数）也不会执行。
    ...
    - ##### 网页状态事件：
      ...
      - `DOMContentLoaded` 事件：
        ...
        注意，网页的 JS 脚本是同步执行的，脚本一旦发生堵塞，将推迟触发`DOMContentLoaded`事件。
  ...
  - #### `GlobalEventHandlers` 接口
    ...
    - ##### `GlobalEventHandlers.o5nerror`：
  
      `error`事件发生时，就会调用`onerror`属性指定的回调函数。
  
      `error`事件分成两种。
  
      一种是 JS 的运行时错误，这会传到`window`对象，导致`window.onerror()`。
