...
- #### 单个参数：（这种情况下mybatis可以做类型的自动推断，所以sql中可以不用指定parameterType属性的值）
  ...
  1. **单个简单类型参数**：byte/short/int/long/float/double/char，以及它们各自的包装类型，还有String、java.util.Date、java.sql.Date，
  2. **单个Map类型**：#{}里写map的key，注意**map的key只能是String类型**。
  3. **单个pojo类**：#{}里写属性名，底层调用了pojo类的get方法进行的传值。

  > **其实#{}中的完整写法是这样**：**#{name, javaType=String, jdbcType=VARCHAR}**，指定该name属性传过来的值类型为String，对应数据库中的类型是varchar。其实这两个包括sql标签中的parameterType属性，都是用来帮助mybatis进行类型确定的。不过这些配置多数是可以省略的，因为mybatis它有强大的自动类型推断，所以了解即可。

  > 实际上mybatis中内置了许多常用类的别名，java.lang.Long就可以直接写为long就行。
...
- #### 多个参数：
  ...
  - 但是现在我们自己不写Mapper实现类了，直接用Mybatis生成的，那么底层原理和上面类似。mybatis会帮我们将接口中传过来的多个参数，先进行处理。它底层会创建一个**map<String,Object>**，将接口的多个参数封装到这个map中，并且该map的key是**#{arg0}**、#{arg1}...或**#{param1}**、#{param2}..这样的形式*（注意：如果是mybatis3.4.2之前的版本，要用#{0}和#{1}这种方式取出来）*
  ...
  ###### 如果形参是一个数组类型的，mybatis同样会封装到map中，取用#{array}、#{arg0}来取，建议手动加@Param()注解

------

### MyBatis查询专题

- 我们之前用sqlSession的select方法查询时，返回的结果分别用pojo类、List<T>集合来接收；但如果没有合适的pojo类，可以用map集合来接收单个数据，字段名作map的key(String)，值做map的value，一个map对应一条查询结果；如果有多条结果可以用List<Map>来接收。
...
- 我们之前查询时发现，pojo类属性名和结果集的字段名对不上，还需要用as起别名，这里有更好的方式：
  ...
  - 开启**驼峰命名自动映射**。在mybatis-config中配置：
  ...
  - **resultMap**结果映射：在sql语句的mapper文件中通过配置<resultMap>结果映射标签，然后在<select>标签中添加resultMap属性，指向<resultMap>标签的id
...
- <if>标签：
  ...
  ```txt
  关于if标签有以下几点说明：
      1、其中if标签的test属性是必须的，值是false或true；如果test为true，则if标签中内容会拼接，反之则不会
      2、test属性中，要取传过来的参数Car对象的属性，直接写属性名即可，不需要加#{}或${}；标签里是动态拼接的内容，一定要加#{}
...
- <where>标签：它的作用是让where子句更加动态智能；当where标签中有内容时，会自动生成where关键字，并且会自动去除内容**前面**多余的and或or
  ...
  <select id="selectByMultiCondition" resultType="car">
  	select * from t_car
  	<where>		//where标签时专门负责where子句的动态生成的
  		<if test="brand != null and brand != ''">
  			and brand like "%"#{brand}"%"		//最前面的这个and/or关键字会根据需去掉
  		</if>
  		<if test="guidePrice != null and guidePrice != ''">
  			and guide_price >= #{guidePrice}
  		</if>
...
- <set>标签：主要使用在update语句当中，用来生成set关键字，同时去掉最后多余的逗号
...
- <choose>、<when>、<ohterwise>标签：这三个标签是在一起使用的，类似于if else，比多个<if>标签更方便，只有一个分支会被选择；（注意：<when>最少出现1次，<ohterwise>最多出现1次）
...
1. 首先准备数据库表，t_clazz和t_student，两站表之间有外键关系，一个班级对应多个学生：

   ![image-20240329234818058](./assets/image-20240329234818058.png)

   ![image-20240330173719652](./assets/image-20240330173719652.png)

2. 创建两个pojo类Clazz（除了cid和cname，还有List<Student>属性）和Student（sid、sname和Clazz属性），创建mapper接口StudentMapper、ClazzMapper，创建mapper映射文件StudentMapper.xml、ClazzMapper.xml，还有两个Mapper接口的测试用例，ClazzMapperTest和StudentMapperTest
  ...
  2. ###### 分步查询：（优点：不用写链接查询了，代码复用性增强，且支持懒加载提高执行效率）
    ...
    *懒加载：暂时不访问的数据先不查，提高程序的执行效率。*
    ...
    <resultMap id="selectStep1ResultMap" type="Student">
        <id column="sid" property="sid"/>
        <result column="sname" property="sname"/>
        <!-- 将cid传给第2步的sql，select指向第2步的sqlId，查到的结果对象给clazz属性 -->
    ...
    > 怎么开启懒加载：
    >
    > 在<association>标签中加属性**fetchType="lazy"**，这是局部的懒加载，全局的在mybatis核心配置文件中开启：
    >
    > ```xml
    > <!-- 全局设置标签 -->
    > <settings>
...
- ##### 当班级表为主表时（一对多）：
  ...
  2. ###### 分布查询：
    ...
    <resultMap id="selectStep1ResultMap" type="Clazz">
        ...
        <!-- 将cid传给第2步的sql，select指向第2步的sqlId，查到的结果给students属性 -->
