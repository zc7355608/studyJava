...
- ##### mybatis缓存包括：
  ...
  - 二级缓存：将查询到的数据存储到SqlSessionFactory中。也可以集成第三方的缓存去代替自带的二级缓存：比如EhCache【Java语言开发】、Memcache【C语言开发】等。
  - 区别：范围不同。一级缓存是SqlSession对象，是局部的；二级缓存是SqlSessionFactory对象，数据库级别；
  
- **什么时候不会走缓存，缓存失效**：

  - 不同的SqlSession对象，一级缓存范围肯定不同，或者不同的select语句，查询条件不同，肯定也不走缓存。
  - 两次select中间，进行了任何增删改操作，一级二级缓存都会失效。（哪张表没关系，只要有DML操作）
...
- ##### 缓存的使用：
  ...
  - **二级缓存**：二级缓存的范围是在SqlSessionFactory内，也就是数据库范围，使用二级缓存需要同时具备以下几个条件：
    1. 核心配置文件中配置<setting name="cacheEnabled" value="true"/>，全局性地开启或关闭所有Mapper文件中已配置的任何缓存；默认就是true，无需设置。
    2. 在需要使用二级缓存的SqlMapper.xml文件中添加配置：<cache/>
    3. 使用二级缓存的pojo对象必须是可序列化的，也就是必须实现Serializable接口。
...
2. 在类路径resources下新建echcache.xml文件：
...
3. 最后，在SqlMapper.xml文件中的<cache/>标签中添加type属性即可：
...
```txt
所谓的逆向工程是：根据数据库表逆向生成Java的pojo类、XxxMapper.xml文件、Mapper接口。
要完成这个工作，需要借助maven中别人写好的逆向工程插件：
...
### MyBatis中使用PageHelper分页插件

> 我们正常写分页功能时，都是要向后端发送【页码】和【每页显示条数】这2数据的，后端获取之后，将表中所有数据通过limit截取后返回。假设每页显示pageSize条，第pageNum页的数据应该怎么用limit来获取呢？可以这样：
>
> 【startIndex = (pageNum - 1) * pageSize】

###### 获取数据不难，难的是获取分页相关的数据比较难。此时可以借助mybatis的PageHelper插件

> PageHelper是别人写好的一个插件，可以在MyBatis中更方便的使用分页查询，不局限于用limit拼接进sql进行分页。并且它的性能也很不错。
...
3. 编写测试代码：
   ...
   //一定在查之前开启分页
       ...
       // 执行查询语句
       List<Car> cars = mapper.selectByExample(null);
       // 最后一定要将结果封装成PageInfo分页信息对象，第二个参数是导航卡片数量
