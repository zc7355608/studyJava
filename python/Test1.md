...
- ### Python基础语法
  ...
  - Python中没有switch语句，只有match语句。match语句中依次用`case xxx`匹配，并且可以在最后（且仅能在最后）加一个`case _`表示“任意值”：
...
- ### Python的数据类型（字面量）
  ...
  - 字节序列：Python对`bytes`类型（字节序列）的数据用带`b`前缀的单引号'或双引号"表示，如：`x = b'ABC'`，其中每个字符占1字节。

    > 字符串和字节序列分别有`encode('utf-8')`和`decode('ascii')`方法可以编码和解码。
  ...
  - 字典（dict）：Python内置了字典dict的支持，全称dictionary，对应Java中的map，使用键-值（K-V）存储数据，具有极快的查找速度。定义dick：
    ...
    > - 或者用字典的`get()`方法，若key不存在，返回`None`或自己指定的值：`d.get('Thomas', '不存在')`
    > - 从字典中删除元素：`d.pop(K)`。

  - Set：set类似Java中的set集合，无序不可重复，只存储Key。如：`s = {'a', Ture, 123}`。
...
- ### 函数
  ...
  - ##### 函数的参数：
    ...
    - **默认参数**：类似JS。注意：Python中定义默认参数时，形参必须指向一个不可变的值！（**默认参数的副作用问题**）
    ...
    - **关键字参数**：关键字参数允许你传入0个或任意个含参数名的参数，这些关键字参数在函数内部自动组装为一个dict。如：
    ...
    - **命名关键字参数**：如果要限制关键字参数的名字，就可以用命名关键字参数，例如，只接收`city`和`job`作为关键字参数。这种方式定义的函数如下：
      ...
      >
      > - 调用：`person('Jack', 24, city='Beijing', job='Engineer')`，**调用时必须指定实参名**。
      >
    ...
    ###### 以上几种形式的函数参数可以任意组合使用。但是注意，参数定义的顺序必须是：普通形参、默认参数、可变参数、命名关键字参数和关键字参数。
  
  - ##### Python内置了很多有用的函数，我们可以直接使用：
