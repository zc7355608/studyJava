  ...
  - #### Loader 原理
    ...
    - ##### loader 分类
      ...
      2. ###### 异步 loader
         ...
         module.exports = function (content, map, meta) {
           ...
           setTimeout(() => {
             ...
             /*
             this.callback();
             注意：同步loader中不能进行异步操作。也就是setTimeout里面不能调用同步loader的回调this.callback()，会报错的。因为你上一个loader没有返回值，所以后面的loader的content接收到的是undefined
             */
           }, 1000);
         };
         ```
      ...
      4. ###### Pitching Loader
         ...
         >
         > 在这个过程中如果任何 pitch 函数有返回值（不为undefined），则 loader 链被阻断。webpack 会跳过后面所有的的 pitch 和 loader，直接进入上一个 loader 。如果没有上一个loader函数，那么当前处理就完成了。
         >
  ...
  - #### Plugin 原理
    ...
    - ##### 开发一个插件
      ...
      3. ###### 启动调试
         ...
         2. 运行指令：`npm run debug`。此时控制台输出以下内容：
         ...
         3. F12 打开浏览器调试控制台。此时控制台会显示一个绿色的图标，点击绿色的图标进入调试模式。在需要调试代码处用 `debugger` 打断点，代码就会停止运行，从而调试查看数据情况。
