...
- ### 第一个React程序
  ...
  > 我们先来做一个React程序，这里先用React的旧版本（16.8）。
  ...
  > - JSX来到世界上只为解决一个问题：创建虚拟DOM太繁琐了。有了JSX可以让编码人员更加简单的创建虚拟DOM，写起来更加流畅。
...
- ### 虚拟DOM与真实DOM
  ...
  > - 我们通过在控制台上打印VDOM发现，它其实就是一个普通的JS对象。也就是说，JSX最终会被Babel转化为JS对象（虚拟DOM）。
...
- ### JSX
  ...
  1. 如果XML标签要动态化，需要混入JS表达式时，要用`{}`包起来。
  2. 如果`{}`里面的是数组，那么React会自动帮你遍历数组，数组中每个元素都当作虚拟DOM顺序放在`{}`所在位置上。
  3. 和HTML中不同的是，标签的class类名属性叫`className`，因为JSX中包含JS语法，而`class`是JS中的关键字。
  4. 和HTML中不同的是，标签的`style`样式属性的值不能用字符串，也就是不能这样写：`<span style='container'></span>`，值必须是JS中的对象（动态的），如：`<span style={ {backgroundColor:'red',color:'red'} }></span>`，属性名采用小驼峰形式。
  5. JSX中**根标签只能有一个**，且每个**标签必须闭合**。
  6. JSX中可以写任意名字的标签。如果是小写字母开头的标签，会被当作HTML标签；如果是大写字母开头的标签，会被当作组件去渲染。
...
- ### React面向组件编程
  ...
  > - 组件的状态驱动着页面。状态中保存着响应式的数据，如果数据变化了组件所对应的HTML页面也会随之更新。
  > - 所谓组件的状态就是**组件实例对象上的`state`属性**，他是组件实例对象的三大核心属性之一。
