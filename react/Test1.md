...
- ### 使用Vite搭建一个React应用
  ...
  - `src/`：存放项目源代码。
    ...
    未来`src/`下还会新建其他目录，比如`components/`中存放（除了 `App` 之外的）**UI 组件**，`layout/`存放**布局组件**。其中每个组件都是一个单独的目录，目录中的`index.jsx/组件名.jsx`就是组件文件，处此之外还有组件的样式`index.css/组件名.css`、组件依赖的静态资源等。
