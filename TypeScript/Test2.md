...
- ### 类
  ...
  - **类的类型：**TS 中的类本身也是一种类型。
    ...
    > ```
    >
    > PointClass变量的类型是Point，但是前面要加上typeof，表示取Point类的类型。
    >
    > ```ts
...
- ### 抽象类
  ...
  - 抽象成员不能被`private`修饰，否则无法在子类中实现该成员。
...
- ### 泛型
  ...
  - **类型参数的约束条件：**
    ...
    > ```
    >
    > 类型参数可以同时设置约束条件和默认值，前提是默认值必须满足约束条件（不能用`super`）。
    >
...
- ### TS 模块
  ...
  - #### TS的模块语法：
    ...
    > TS 默认支持 ES 模块化语法，还允许输出和输入类型。TypeScript 编译器会根据`tsconfig.json`中的`module`配置选，将 TS 模块代码编译为 ES6、CommonJS 或 AMD 的 JS 模块代码，最终由打包工具（如Webpack）将 JS 模块打包为Bundle。
    ...
    > 但是有时需要知道import导入的是一个类型还是实实在在的变量，因为导入的类型本质上是给TS编译器看的，运行时并没有类型代码。因此为了区分导入的是类型还是变量，TS对ESM语法做了扩展：（这里以分别暴露为例）
    >
  ...
  - #### 模块定位
    ...
    >
    > TS 中有两种模块定位方法：**经典策略**和**Node策略**。只有当编译器选项`module`被设置为`commonjs`时才用Node策略，其他情况下都使用经典策略。（`moduleResolution`、`baseUrl`、`paths`、`rootDirs`这些配置项都会影响TS的模块定位策略）
    >
    > 可以使用编译参数`moduleResolution`指定使用哪一种方法。没有指定定位方法时，就看原始脚本采用什么模块格式。如果模块格式是 CommonJS（即编译时指定`--module commonjs`），那么模块定位采用 Node 方法，否则采用 Classic 方法（模块格式为 es2015、 esnext、amd, system, umd 等等）。

    > **相对路径和模块路径：**
    >
    > 加载模块时，目标模块的路径分为相对路径和模块路径两种。相对路径指的是路径以`/`、`./`、`../`开头的模块（分别相对于项目根、当前路径、上级路径）。模块路径指的是不带有基路径前缀的模块路径。
    >
    > ```ts
    ...
    >
    > - `baseUrl`：指定模块的基准路径的基路径（string）。
    > - `paths`：指定某个模块去哪些路径下查找该模块文件（string[]）。值是一个字符串数组，可以指定多个路径。如果第一个脚本路径不存在，那么就加载第二个路径，以此类推。
    > - `rootDirs`：指定多个项目根目录（string[]）。
    >
    ...
    - ##### 经典策略：
      ...
      > 经典策略以当前脚本的路径作为**基准路径**，计算相对模块的位置。比如，脚本`a.ts`里面有一行代码`import { b } from "./b"`，那么 TypeScript 就会在`a.ts`所在的目录，查找`b.ts`和`b.d.ts`。
      >
      > 至于绝对模块，也是以当前脚本的路径作为起点，一层层查找上级目录。比如，脚本`a.ts`里面有一行代码`import { b } from "b"`，那么就会查找`b.ts`和`b.d.ts`。

    - ##### Node策略：

      > Node 策略就是模拟 Node.js 的模块加载方法。
      >
      ...
      >
      > 绝对模块则是以当前脚本的路径作为起点，逐级向上层目录查找是否存在子目录`node_modules`。比如，脚本文件`a.js`有一行`let x = require("b");`，TypeScript 按照以下顺序进行查找。
      >
