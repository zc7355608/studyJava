...
- ### ~~第一个Vue 2程序~~
  ...
  - 在Vue中，标签中的文本要实现动态化可以用Vue模板的插值语法：`{{JS表达式}}`。注意：**表达式中的函数或属性，得是Vue实例上有的**。（表达式不要过于复杂，模板中也不要用过多的表达式）
...
- ### Vue的数据绑定
  ...
  **双向数据绑定**：`v-model`指令则是双向的数据绑定，绑定的是标签的`value`属性。只要属性值发生变化，对应的`data`中的数据也会变化。它必须用在**有`value`属性的标签**上（表单类标签）。
...
- ### 计算属性和监视属性的区别
  ...
  ###### 注意：所有被Vue管理的方法，不要用箭头函数的写法，这样`this`才是Vue实例或组件实例对象。而不被Vue管理的函数（AJAX回调、Promise回调、定时器回调），最好写成箭头函数，这样this指向的还是当前上下文的Vue实例。
