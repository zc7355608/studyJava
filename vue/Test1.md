  ...
  <body>
      ...
      <script>
          ...
          const v = new Vue({
              ...
              el: '#root',
              // data中的数据会放在Vue实例上，故可以在Vue模板中直接用，值目前先写成对象（该对象中只写属性别写方法）
              data: { user: '尚硅谷' }
          })
          // 修改Vue实例上的user后，页面会随之更新，这就是vue的响应式，它会检测data中的数据
          v.user = '尚学堂'
      </script>
  </body>
  </html>
...
- ### 第一个程序的细节：
  ...
  > - **（重要）Vue对data的响应式的原理**：
  >   - Vue首先对data对象中的数据做了增强，生成了**代理对象**，Vue身上的`_data`属性指向了该代理对象（并且原来指向data的引用也指向了该代理对象）。代理对象的对应属性都是虚拟属性，访问和修改的其实都是data。
  ...
  >   - 为了更方便的操作`_data`代理对象中的数据，Vue将`_data`中的所有属性都放在了Vue实例上一份。实际上Vue实例上的这些属性也都是虚拟属性，也是通过`Object.defineProperty()`方法的`getter`和`setter`加的，访问和修改的都是`_data`中代理对象的数据。
  >
...
- ## Vue的模板语法
  ...
  - #### 插值语法：
    ...
    > 如果是**标签内的文本**要实现动态化，就需要用插值语法，用`{{}}`将JS表达式包起来。

  - #### 指令语法：（指令主要用于`解析标签`，形式都是`v-xxx`，可以解析标签属性、标签体内容、绑定事件等....）

    > 如果标签的**属性值**要实现动态化，需要用指令语法。例如`v-bind`指令：`v-bind:属性名='JS表达式'`，它可以实现标签属性值的动态化。当表达式的结果为`false`，则标签上就没有该属性。（有点像`Thymeleaf`中的`th:属性名="${域中的数据}"`）由于该指令很常用，所以**可以简写**`:属性名='JS表达式'`
