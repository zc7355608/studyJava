...
- ## 事件处理
  ...
  - #### 给元素绑定事件：
    ...
    > - 通过指令`v-on:事件名称='表达式'`（简写：`@事件名称='表达式'`），可以给元素绑定事件，然后在Vue对象的配置里加`methods: {方法}`，里面是需要Vue对象在事件发生时去调用的方法。最终这些方法也会出现在Vue实例上，所以才可以在Vue模版中直接用。
    ...
    > - 表达式中的函数调用还可以加小括号`()`传参，不加调用时默认也会传一个**事件对象**过去。如果要传多个形参，可以用`$event`做事件对象的占位符，避免传其他参数时事件对象消失。
...
- ## 其他的内置指令
  ...
  - `v-clock`（没有值）：该指令是标签中的一个特殊的属性，Vue实例在接管该容器后就会将其删掉。使用场景：
    ...
    > 使用CSS选择器配合`v-clock`可以解决网速慢时，页面显示了未经过Vue编译的Vue模板。我们通常在Vue模板容器上加上该属性，并在`<style>`中添加样式：`[v-clock] {display:none;}`。这样在Vue接管该容器之前，这个容器是不会展示在页面上的。
