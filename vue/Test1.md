...
- ## Vue的数据绑定
  ...
  >
  > - **双向数据绑定**：`v-model`指令则是双向的数据绑定，绑定的是元素的value属性。只要属性值发生变化，对应的data中的数据也会变化。它必须用在**有value属性的表单类元素**上。（只有在自定义的组件标签上，才能指定`v-model:属性名`不绑定value属性，而是绑定其他的属性）
...
- ## 事件处理
  ...
  - #### 给元素绑定事件：
    ...
    > - 表达式中的函数调用还可以加小括号`()`传参，不加调用时默认也会传一个**事件对象**过去。如果要传多个形参，可以用（vue实例上的）`$event`做事件对象的占位符，避免传其他参数时事件对象消失。
...
- ## 条件渲染
  ...
  - `v-show='JS表达式'`：当表达式的值为true时就显示该元素。它相当于设置了`display: true;`，DOM还在HTML中，只是隐藏了。（而`visible: hidden`是隐藏，但页面上还占着位置，display则不会占着位置）
...
- ## 表单数据的收集
  ...
  > - 单选按钮：必须为每个单选按钮指定value属性之后，再写`v-model='gender'`去双向绑定某个变量，否则没value收集到的是null。
  > - 多选框：
  ...
  > - 下拉框：必须给`<select>`标签加`v-model='city'`，而且要为每个下拉选项`<option>`指定value属性。这样city变量才会关联上每个下拉框。
...
- ## 其他的内置指令
  ...
  - **自定义指令**：可以在Vue实例的配置项中通过`directives`来定义自定义指令`v-big`，里面是big指令对应的big函数：
    ...
    > - 指令名定义时不加v-，但使用时要加v-。如果指令名是多个单词，要用短横线风格（HTML属性标准风格）。
    >
