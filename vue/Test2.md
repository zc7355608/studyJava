...
- ## 非单文件组件
  ...
  1. 定义组件：在JS中通过`Vue.extend({})`可以定义一个Vue的组件并返回，参数中的配置对象类似vm中的。注意：
     ...
     >   ```
     > 
     > - `data`只能用函数式写法（Vue3中，建议在任何情况下，data都用函数式写法）。因为对象式写法多个组件实例会共用这个data对象，数据不独立；而函数式写法每次调用会返回一个新的data对象，多个组件实例之间的data互不影响。
...
- ## 单文件组件（常用）
  ...
  1. 先新建一个`.vue`文件（**定义组件**）。该文件的名字同样遵循上面的组件名规范，一般用大写的方式：
     ...
     > - 之前说过，一个Vue组件中包含了HTML、CSS、JS，也就分别对应了单文件组件中的3个标签`<template>`、`<style>`、`<script>`，最终这个`.vue`文件还是会被翻译为一个`.js`文件。（script标签可以有多个，但是多个标签的`lang`属性值必须相同）
...
- ### Vue中的插槽（Vue 2.6.0之后及Vue 3的新用法）
  ...
  2. 使用插槽：（往默认插槽中传递HTML标签）
     ...
     ```
     > - 要往插槽中放的标签结构，它的CSS样式，无论是写在定义插槽的地方、还是使用插槽的地方都行。只是渲染位置不同罢了。
     >
     > - **具名插槽**：组件中可以定义多个`<slot>`插槽，通过其`name='s1'`属性来区分不同的插槽。（没有提供 `name`属性的插槽，默认会有一个name属性，值为`default`。
     >
     >   > - 要为具名插槽传入内容，需要用一个`<template>`标签将传递的结构包起来，并且通过`v-slot:插槽名`属性指定要放入的插槽。（简写形式：`#插槽名`）
     >   >
     ...
     >   >
     >   >   ```vue
     >   >   <template v-slot:[变量]></template>
     >   >   ```
     >
     > - **作用域插槽**：插槽也可以给使用者传递数据，通过给`<slot>`加属性：`<slot :games="g">`。
     >
     >   > - 如果是默认插槽：使用插槽的地方，给组件加`v-slot="data"`属性：（其中data是对象，对象的key是slot标签的属性名games）
     >   >
     >   >   ```vue
     ...
     >   >   ```
     >   >
     >   > - 如果是具名插槽：则必须在具名插槽的template标签上通过`v-slot:插槽名="data"`来获取。（简写：`#插槽名="data"`）
  
  ###### 注意：其实通过组件实例的`vc.$slots.插槽名`也可以获取到的传过来的标签结构，值是一个虚拟DOM数组。使用场景：
  
  > 有时需要判断是否往插槽中传递了结构，如果传递了再渲染某些内容，此时可以使用`$slots`与`v-if`配合来实现：
  >
  > ```vue
  > <template>
